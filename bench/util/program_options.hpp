...
#ifndef UTIL_PROGRAM_OPTIONS_HPP_
#define UTIL_PROGRAM_OPTIONS_HPP_
...
template<po::Style style = po::kStyleSimple>
class ProgramOptions {
 ...
 public:
  ...
  virtual void PrintUsage(const char* argv0, std::ostream* os) const {
 ...
 protected:
  ...
  class Option : public OptionBase {
   public:
    ...
    std::size_t count() const { return val_.is_set(); }
      ...
      void Parse(char** argv, unsigned cnt, int* argind) {
        ...
        if (cnt == 0)
          throw Exception(std::string("Missing value for option '") +
                          argv[*argind - 1] + "'");
        this->ParseToken(argv[(*argind)++], &value_);
        done_ = true;
      }
  ...
  template<typename T>
  static void Set(const T& value, OptionBase* o) {
    o->set(&value);
  }

  template<typename T>
  static void Set(unsigned index, const T& value, MultiOption<T>* o) {
  ...
  template<typename T>
  static void SetIfNot(unsigned index, const T& value, MultiOption<T>* o) {
    if (!o->count())
      Set(index, value, o);
  }

  template<typename T>
  static void Clear(OptionBase* o) {
    o->clear();
  }
 ...
 private:
  ...
  class OptionBase {
   public:
    ...
    void PrintUsage(std::ostream* os) const {
      ...
      *os << name_;
      ...
      do {
        size_t to = ++from;
        for (size_t to_max = desc_.size(); to < to_max && desc_[to] != '\n'; )
          ++to;
        *os << "\n\t\t" << desc_.substr(from, to - from);
        from = to;
      } while ((from = desc_.find('\n', from)) != std::string::npos);
      *os << std::endl;
    }
   ...
   public:
    ...
    char alias() const { return alias_; }
  ...
  class WrappedPointer {
   public:
    ...
    WrappedPointer& operator=(WrappedPointer<T> c) {
...
#endif /* UTIL_PROGRAM_OPTIONS_HPP_ */
